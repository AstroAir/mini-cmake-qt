...
public:
  ...
  cv::Mat merge_images(const cv::Mat &base, const cv::Mat &img1,
                       const cv::Mat &img2) const;

  // 元数据管理
  void
  add_metadata(std::string_view commit_hash,
  ...
  // 修改元数据管理方法
  void
  update_metadata(std::string_view commit_hash,
                  const std::unordered_map<std::string, std::string> &metadata);
...
private:
   ...
   */
  cv::Mat compute_parallel_diff(const cv::Mat &base,
                                const cv::Mat &current) const;
   ...
   */
  void
  process_image_blocks(cv::Mat &image,
  ...
  void update_branch_head(std::string_view branch_name,
                          std::string_view commit_hash);
  cv::Mat find_conflict_regions(const cv::Mat &base, const cv::Mat &img1,
                                const cv::Mat &img2) const;
  ...
  // 缓存管理器
  mutable CacheManager cache_manager_;
};
